...
contract ColorsContract is ERC721Enumerable, Ownable {
    ...
    uint256 public RandomMintCommision = 0;
    ...
    function setRandomMintCommision(uint256 randomMintCommision)
        public
        onlyOwner
    {
        RandomMintCommision = randomMintCommision;
    }
        ...
        onlyOwner
    {
        ...
        for (uint32 i = 0; i < colorCodes.length; i++) {
            if (!_exists(colorCodes[i]) && colorCodes[i] <= MaxColors) {
    ...
    function margeColors(
        ...
        require(iterationCount >= 0, "iterationCount need minimum 0");
        require(msg.value >= MargeCommision, "value != MargeCommision");

        require(totalSupply() + 1 <= MaxColors, "Max supply of Colors");
        require(_exists(colorCode0), "colorCode0 not exist");
        require(_exists(colorCode1), "colorCode1 not exist");
    ...
    function predictColorAfterMarge(
        ...
        require(iterationCount >= 0, "iterationCount need minimum 0");
        require(colorCode0 != colorCode1, "colorCode0 != colorCode1");
    ...
    function setInfo(
        ...
        require(_exists(colorCode), "Color dosen't exist");
     ...
     * iterationCount - количество итераций которые нужно сделать когда цвет не уникальный
     */
    function mintRandomColor(uint32 iterationCount)
        public
        payable
        returns (uint32)
    {
        ...
        require(iterationCount >= 0, "iterationCount need minimum 0");
        require(
            msg.value >= RandomMintCommision,
            "value != RandomMintCommision"
        );
        ...
        // Не удалось сгенерировать уникальный цвет
        // нужно прервать выполнение контракта
        require(generationSeccsess == true, "generationSeccsess != true");
