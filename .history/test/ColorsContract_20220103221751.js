...
contract('ColorsContract', (accounts) => {
    ...
    it('Mint with problems', async () => {
        ...
        // mintRandomColor -1 
        try {
            var result = await ColorsContractInstance.mintRandomColor(-1, {
                from: callerAccount
            });
    ...
    it('Mint random colors', async () => {
        const ColorsContractInstance = await ColorsContract.deployed();
        var callerAccount = accounts[0];
        await ColorsContractInstance.mintRandomColor(0, {
            from: callerAccount
        });
        await ColorsContractInstance.mintRandomColor(10, {
            from: callerAccount
        });
        await ColorsContractInstance.mintRandomColor(100, {
            from: callerAccount
        });
        var count = await ColorsContractInstance.balanceOf(callerAccount);
    ...
    it('[mintColors] and [predictColorAfterMarge] than show result', async () => {
        ...
        var mint = await ColorsContractInstance.mintColors([16777215, 16777214, 0],
            ["16777215 test1", "16777214 test2", "0"], {
                from: callerAccount
            });
    ...
    it('[setInfo] and [getInfo] than show result', async () => {
        ...
        await RoksTokenInstance.approve(ColorsContractInstance.address, commision * 2);

        await ColorsContractInstance.setInfo(16777215, "new title", "metadtata", {
            from: callerAccount
        });

        var token = await ColorsContractInstance.tokenOfOwnerByIndex(callerAccount, 0, {
            from: callerAccount
        });
        await ColorsContractInstance.setInfo(token, "token color name", "");

        var firstInfo = await ColorsContractInstance.getColorInfo(16777215);
        console.log(firstInfo);
        assert.equal(firstInfo.title, "new title",
            '16777215 title have to \"new title\"');

        var firstInfo1 = await ColorsContractInstance.getColorInfo(token);
        console.log(firstInfo1);
        assert.equal(firstInfo1.title, "token color name",
            'token title have to \"token color name\"');

        console.log("       roksHolderAccount balanceOf " +
            (await RoksTokenInstance.balanceOf(roksHolderAccount)));
        console.log("       callerAccount balanceOf " +
            (await RoksTokenInstance.balanceOf(callerAccount)));

        assert.equal((await RoksTokenInstance.balanceOf(roksHolderAccount)), 500,
            'roksHolderAccount balanceOf have to 500');
        assert.equal((await RoksTokenInstance.balanceOf(callerAccount)), 500,
            'callerAccount balanceOf have to 500');
